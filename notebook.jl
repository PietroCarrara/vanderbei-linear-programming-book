...
# ╔═╡ fe1b3ffb-dc94-4076-a1fc-ca3c6f5e6728
begin
	using Plots
	using Printf
end

# ╔═╡ f4f847c7-e47a-4afa-a33a-4441bb5dc3a2
# Utils
begin
	square(size, bl = (0, 0)) = Shape([bl, (bl[1], bl[2]+size[2]), (bl[1]+size[1], bl[2]+size[2]), (bl[1]+size[1], bl[2])])
end

# ╔═╡ 9847674a-224a-11ec-185e-bfb2e3fdfa26
md"
#### 1.1
...
```
	...
	Coils 4,000 tons
```
Given that there are 40h of production time available this week, the problem is to decide how many tons of bands and how many tons of coils should be produced to yield the greatest profit. Formulate this problem as a linear programming problem. Can you solve this problem by inspection?
"

# ╔═╡ c25e162c-8356-4f19-82e5-ca2a36e153e8
md"
...
# ╔═╡ 65cb5b0c-9ba2-4029-aeeb-a3291e3b61e5
begin
	plot(; xlabel="Coils", ylabel="Bands", lims=(0, 7000))
	
	plot!(square((4000, 6000)), c = plot_color(:blue, 0.1))
	vline!([4000])
	hline!([6000])
	
	# Transform b/200 + c/140 <= 40 into a function and plot it
	bands_given_coils(c) = (40 - c/140)*200
	plot!(bands_given_coils, [0, 7000])
	
	# Transform b/200 + c/140 <= 40 into a function
	coils_given_bands(b) = (40 - b/200)*140
	
	profit(c, b) = @sprintf("\$%.2f", 30c + 25b)
	
	# Mark the intersections
	points = [
		(4000, bands_given_coils(4000)),
		(coils_given_bands(6000), 6000),
	]
	scatter!(points, series_annotations = (points .|> p -> profit(p[1], p[2])))
end

# ╔═╡ 39150562-fe78-40c9-afcf-a664941fe20c
md"
#### 1.2
...
# ╔═╡ dbae324d-459e-4fcd-9c81-5c9ac8bdcf16
md"
Let $R = \{\text{Ithaca-Newark}, \text{Newark-Boston}, \text{Ithaca-Boston}\}$,

Let $C = \{\text{Y}, \text{B}, \text{M}\}$,

Let $p_{c,r}, r \in R, c \in C$ be the **price** of a ticket of route $r$ in class $c$, as shown in the first table,

Let $u_{c,r}, r \in R, c \in C$ be the **upper bound** of passengers of route $r$ in class $c$, as shown in the second table,

Let $q_{c,r}, r \in R, c \in C$ be the **quantity** of tickets of route $r$ in class $c$ we'll sell (the *decision variable*):

$\text{maximize}\ \sum_{c,r} p_{c,r}q_{c,r}$
$\text{subject to}$
$q_{c,r} <= u_{c,r}$
$\sum_{c} q_{c,r} <= 30$
"

# ╔═╡ 8c3c34d7-ebfb-49c4-8e52-65f6e2913673
md"
#### 1.3
...
Of course, the numbers $p_j, j = 1, 2, ..., n$ are nonnegative and sum to one. The same is true for the $qj$’s. Based on a single observation of $Y$, we wish to guess whether it has distribution $p$ or distribution $q$. That is, for each possible outcome $a_j$, we will assert with probability $x_j$ that the distribution is $p$ and with probability $1 − xj$ that the distribution is $q$. We wish to determine the probabilities $x_j, j = 1, 2, ..., n$, such that the probability of saying the distribution is $p$ when in fact it is q has probability no larger than $\beta$, where $\beta$ is some small positive value (such as $0.05$). Furthermore, given this constraint we wish to maximize the probability that we say the distribution is $p$ when in fact it is $p$. Formulate this maximization problem as a linear programming problem.
"

# ╔═╡ a3498419-d5f9-42e2-9ca9-ba52ab99f49b
md"
$\text{\textit{Can't solve}}$
"
